...
print_sndvolume() {
	...
	VOLUME=`/usr/bin/mixerctl outputs.master| cut -d "=" -f2|awk -F, '{print $1"/"$2}'`
  	VOLUME_STATUS=`/usr/bin/mixerctl outputs.master.mute`
	if [ $VOLUME_STATUS = "outputs.master.mute=on" ]; then
		echo -n " -SND: $VOLUME"
	else
		echo -n " +SND: $VOLUME"
	fi 
